...
void tSerial(void *p) {
	while(1){
		...
		xQueueSendToBack(queue, (void *) &blueToothVal, (TickType_t) 90);
		vTaskDelay(1);
...
void tMotorControl(void *p) {
	while(1){
		char value;
		if(xQueueReceive(queue, &value, (TickType_t) 90) && (lastValue != value)
				&& (xSemaphoreTake(semaphore, (TickType_t) 90) == pdFALSE)
				&& (xSemaphoreTake(semaphoreY, (TickType_t) 90) == pdFALSE)){
				lastValue = value;
...
void tLED(void *p) {
	while(1) {
		char valueRun;
		if(xQueueReceive(queue, &valueRun, (TickType_t) 90) ){
...
void tAudio(void *p) {
	while(1) {
		char valueAudio;
		if(xQueueReceive(queue, &valueAudio, (TickType_t) 90)){
			...
			else if(connectionEstablished && valueAudio != 'F') {
				lastBabyShark = millis();
				mazeAudio();
			}
		}
